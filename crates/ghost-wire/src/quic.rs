...
use quinn::{Endpoint, Incoming, ServerConfig, ClientConfig, TransportConfig};
...
pub async fn quic_echo_oneshot_server<A: ToSocketAddrs>(bind: A) -> Result<(SocketAddr, Vec<u8>, tokio::task::JoinHandle<()>), QuicError> {
	...
	let task = tokio::spawn(async move {
		if let Some(connecting) = incoming.into_iter().next() {
			if let Ok(conn) = connecting.await {
				if let Ok((mut send, mut recv)) = conn.accept_bi().await {
					// Read a length-prefixed message and echo it back
